...
1. > I want you to read this github link with instructions and rubric for my lab and then help me complete it for applied computing in health data science PUBH4021. Always ask questions if you have any never assume anything. DO NOT START OR DO ANYTHING WITHOUT MY INSTRUCTION. DO NOT MAKE ANYTHING UP. 

2. > For design and project choices always ask me. I want to work on the genomics because its most interesting to me. NEVER ASSUME YOU KNOW EVERYTHING.  
...
## Why I chose Welch's t-test and BH-FDR

Welch's t-test: I used Welch's t-test instead of a standard Student's t-test because it doesn't assume the two groups (Dex-treated vs. untreated) have equal variance and with a drug treatment, there's no reason to assume it would leave the spread of gene expression unchanged even if it changes the average. With only 4 replicates per group there's no reliable way to test that assumption first, so defaulting to the version that doesn't require it is the safer choice, at only a negligible power cost if variances did happen to be equal. I also ruled out a non-parametric test (Mann-Whitney U) since with n = 4 per group it can't be significant, and a paired test wasn't possible because this processed data file doesn't preserve which treated and untreated sample came from the same original cell line.

Benjamini-Hochberg FDR correction: Since this analysis runs a separate statistical test on each of ~12,000 genes, using a raw p < 0.05 cutoff would produce roughly 600 false positives by chance alone, so some multiple-testing correction is required. I chose Benjamini-Hochberg over the stricter Bonferroni correction because Bonferroni controls the chance of even a single false positive across the whole experiment, which is overly conservative for exploratory work like this and would discard many real effects, especially since gene expression levels are often correlated with each other. BH-FDR instead controls the expected proportion of false positives among the genes called significant, which is a more practical guarantee for generating a trustworthy shortlist of candidate genes and it's the actual field-standard default used by real differential expression tools like DESeq2, edgeR, and limma.

## What was mine vs. AI-generated

The code, analysis choices (expression filter threshold, Welch's t-test, BH-FDR, which known genes to check), and all design choices in the notebook and this file were built with Claude under my direction. My own contributions were the decisions documented in the prompt log above dataset choice, scope, catching errors, working through bugs, and checking over the rubric and instructions to make sure its done correctly. I also had Claude explain and recommend Welch's t-test and BH-FDR tests because I never learned about them before, but me choosing them at the end was still my choice. 
